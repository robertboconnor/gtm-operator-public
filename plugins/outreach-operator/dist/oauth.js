 ...
 */
export function toStoredTokens(response) {
    ...
    for (;;) {
        try {
            ...
            if (Date.now() > deadline) {
                throw new OutreachAuthError(`Timed out waiting for the Outreach token lock at ${LOCK_DIR}. If no other run is in flight, delete that directory and retry.`);
            }
...
async function postToken(params) {
    ...
    if (!response.ok) {
        throw new OutreachAuthError(`Outreach token request failed (${response.status} ${response.statusText}): ${body}`);
    }
...
async function refreshTokens(current) {
    if (nowSeconds() >= current.refresh_expires_at) {
        throw new OutreachAuthError("The Outreach refresh token has expired (they last 14 days). Re-authorize with: npm run login --prefix plugins/outreach-operator");
    }
    const response = await postToken({
        client_id: requireEnv("OUTREACH_CLIENT_ID"),
        client_secret: requireEnv("OUTREACH_CLIENT_SECRET"),
...
export async function getAccessToken() {
    ...
    if (!existing) {
        throw new OutreachAuthError(`No Outreach tokens found at ${TOKEN_PATH}. Authorize once with: npm run login --prefix plugins/outreach-operator`);
    }
    if (isFresh(existing)) {
...
export async function hasScope(resource, level) {
