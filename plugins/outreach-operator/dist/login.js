...
async function waitForCode(redirectUri, expectedState) {
...
`);
    ...
    console.error(`Refresh token valid until ${new Date(tokens.refresh_expires_at * 1000).toISOString()} (14 days; it renews every time the tools are used).`);
}
main().catch((error) => {
