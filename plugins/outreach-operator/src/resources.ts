...
export interface ResourceSpec {
  /** Collection path segment on /api/v2. */
  path: string;
  /**
   * The JSON:API `data.type` for writes. Spelled out rather than derived: the
   * plurals are irregular (opportunities -> opportunity, mailboxes -> mailbox),
   * and a wrong type is rejected by Outreach on every create and update.
   */
  type: string;
  label: string;
  creatable: boolean;
...
export function resolveResource(name: string): ResourceSpec {
  const spec = RESOURCES[name];

  if (!spec) {
    throw new Error(
      `Unknown Outreach resource "${name}". Supported: ${Object.keys(RESOURCES).sort().join(", ")}`,
    );
  }
...
export async function resourceGet(input: { resource: string; id: number | string }) {
  const spec = resolveResource(input.resource);
...
export async function resourceCreate(input: {
  ...
  relationships?: Record<string, { type: string; id: number | string }>;
}) {
  const spec = resolveResource(input.resource);
  ...
  const record = await outreachCreate(
    spec.path,
    spec.type,
    input.attributes,
    buildRelationships(input.relationships),
  );
...
export async function resourceUpdate(input: {
  ...
  if (!spec.updatable) {
    throw new Error(
      `Outreach does not support updating a ${spec.label} via the API. Delete and recreate it instead.`,
    );
  }

  const record = await outreachUpdate(
    spec.path,
    spec.type,
    input.id,
    input.attributes,
...
export async function resourceDelete(input: { resource: string; id: number | string }) {
  const spec = resolveResource(input.resource);
...
async function assertScope(
  ...
  throw new Error(
    `This Outreach connection was not granted "${resource}.${level}", so ${action} is not possible. Tick it on the app in the Outreach developer portal and re-run the login, or ask whoever owns the app to.`,
  );
}
...
function buildRelationships(
  ...
  return Object.fromEntries(
    Object.entries(relationships).map(([name, ref]) => [
      name,
...
export async function customFieldTypes() {
  const document = await outreachRequest<Record<string, unknown>>("/types");
