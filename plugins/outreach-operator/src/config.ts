...
export const CREDENTIAL_DIR = path.join(
  os.homedir(),
  ".config",
  "gtm-operator",
  "outreach",
...
export function requireEnv(name: string) {
  ...
  if (!value) {
    throw new Error(
      `Missing ${name}. Put it in ${envPath} before running the Outreach Operator plugin.`,
    );
  }
...
export function assertWritable(operation: string) {
  if (isReadOnly()) {
    throw new Error(
      `GTM_READONLY is set, so ${operation} is blocked. Unset GTM_READONLY to allow Outreach mutations.`,
    );
  }
}
