...
}): Promise<{ names: string[]; scope: string }> {
...
async function fetchHistory(input: {
...
}): Promise<Record<string, PropertyVersion[]>> {
  const merged: Record<string, PropertyVersion[]> = {};
  ...
  for (let index = 0; index < input.propertyNames.length; index += HISTORY_CHUNK_SIZE) {
    ...
    const response = await hubspotRequest<{
      results?: { propertiesWithHistory?: Record<string, PropertyVersion[]> }[];
 ...
 */
function extractColumnMappings(raw: Record<string, any>) {
  const fileImports: any[] = raw?.importRequestJson?.fileImports ?? [];

  return fileImports.flatMap((file) =>
    (file?.pages ?? []).flatMap((page: any) => page?.columnMappings ?? []),
  );
}

const importCache = new Map<string, ImportSummary>();

async function fetchImportSummary(importId: string): Promise<ImportSummary> {
  ...
  try {
    const raw = await hubspotRequest<Record<string, any>>(`/crm/v3/imports/${importId}`);
    ...
    const summary: ImportSummary = {
      importId,
      unavailableReason:
        error instanceof Error ? error.message : "Import metadata unavailable.",
    };
  ...
  try {
    ...
    if (!names.length) {
      return makeErrorEnvelope({
        operation,
        error: new Error(
          "No properties matched. Pass properties[] explicitly or widen propertySearch.",
        ),
        audit: {
          attempted: false,
          verified: false,
    ...
    let imports: ImportSummary[] | undefined;
...
export async function crmImportHistory(input: {
  ...
  try {
    ...
    const byImport = new Map<
      string,
      { timestamps: string[]; values: Record<string, { value: string; setAt: string }> }
    >();
    ...
    return makeEnvelope(
      ...
      },
      {
        ...
        note:
          "Reconstructed from per-property change history (sourceType=IMPORT). Only properties that this import actually wrote appear under values.",
        imports,
      },
    );
  } catch (error) {
...
export async function importsGet(input: { importId: string }): Promise<ToolEnvelope> {
  const operation = "imports.get";

  try {
    const raw = await hubspotRequest<Record<string, any>>(
      `/crm/v3/imports/${input.importId}`,
    );
...
export async function importsList(input: {
  ...
  try {
    ...
    const response = await hubspotRequest<{ results?: Record<string, any>[] }>(
      `/crm/v3/imports?${params.toString()}`,
    );
    ...
    return makeEnvelope(
      operation,
      { attempted: true, verified: true, targetType: "import" },
      {
