...
import { exactWorkflowMatch, makeEnvelope, makeErrorEnvelope, nextActionIdFromWorkflow, sanitizeWorkflowForCreate, sanitizeWorkflowForUpdate, } from "./utils.js";
...
export async function getWorkflow(flowId) {
    return hubspotRequest(`/automation/v4/flows/${flowId}`);
}
export async function createWorkflow(payload) {
    return hubspotRequest("/automation/v4/flows", {
        method: "POST",
        body: JSON.stringify(payload),
    });
}
export async function updateWorkflow(flowId, payload) {
    return hubspotRequest(`/automation/v4/flows/${flowId}`, {
        method: "PUT",
        body: JSON.stringify(payload),
    });
}
export async function deleteWorkflow(flowId) {
    return hubspotRequest(`/automation/v4/flows/${flowId}`, {
        method: "DELETE",
    });
}
export async function workflowsSearch(input) {
    ...
    if (input.exactName) {
        ...
        matches = workflows.filter((workflow) => workflow.name?.trim().toLowerCase() === target);
    }
    else if (input.query) {
        const target = input.query.trim().toLowerCase();
        matches = workflows.filter((workflow) => workflow.id === input.query ||
...
export async function workflowsGet(input) {
    ...
    if (input.workflowId) {
        try {
            ...
            const summary = {
                ...
                objectTypeId: typeof workflow.objectTypeId === "string" ? workflow.objectTypeId : undefined,
...
export async function workflowsAddGoToWorkflowStep(input) {
    ...
    try {
        ...
        try {
            ...
            const shouldTreatAsUnsupported = !!hubspotError &&
