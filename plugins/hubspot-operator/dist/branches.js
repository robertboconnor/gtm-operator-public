...
function outgoingActionIds(action) {
    const out = [];
    const push = (edge) => {
 ...
 */
function collectChain(actionsById, rootId) {
    const seen = new Set();
    const stack = [rootId];
...
function remapActionIds(value, idMap) {
...
function branchFilterInfo(branch) {
    const values = [];
    let property;
    const walk = (node) => {
...
function findTemplateBranch(actions, templateValue) {
...
export async function workflowsAddEmailBranch(input) {
    ...
    const actions = Array.isArray(workflow.actions)
        ? workflow.actions.filter(isRecord)
        : [];
    ...
    if (existing) {
        return makeErrorEnvelope({
            operation,
            error: new Error(`A branch matching ${JSON.stringify(input.matchValue)} already exists in this workflow.`),
            audit,
        });
    }
    ...
    if (!host) {
        return makeErrorEnvelope({
            operation,
            error: new Error(`No branch matching ${JSON.stringify(input.templateMatchValue)} was found to clone.`),
            audit,
            data: {
                availableBranchValues: actions
                    .filter((action) => Array.isArray(action.listBranches))
                    .flatMap((action) => action.listBranches.flatMap((branch) => branchFilterInfo(branch).values)),
            },
        });
    }
    const hostBranches = host.action.listBranches;
    const templateBranch = hostBranches[host.branchIndex];
    const templateRoot = isRecord(templateBranch) &&
    ...
    const emailActionIds = chainIds.filter((id) => actionsById.get(id)?.actionTypeId === SEND_EMAIL_ACTION_TYPE);
    if (emailActionIds.length !== input.emailIds.length) {
        return makeErrorEnvelope({
            operation,
            error: new Error(`Template branch sends ${emailActionIds.length} email(s) but ${input.emailIds.length} emailId(s) were supplied. ` +
            ...
            data: {
                ...
                templateEmailIds: emailActionIds.map((id) => actionsById.get(id)?.fields?.content_id),
            },
        });
    }
    // Allocate fresh ids for the cloned chain.
    const startId = Number.parseInt(typeof workflow.nextAvailableActionId === "string"
        ? workflow.nextAvailableActionId
        : String(Math.max(0, ...actions.map((a) => Number.parseInt(String(a.actionId), 10) || 0)) + 1), 10);
    const idMap = new Map();
    chainIds.forEach((oldId, index) => {
        idMap.set(oldId, String(startId + index));
    });
    const clonedActions = chainIds.map((oldId) => {
        const cloned = remapActionIds(actionsById.get(oldId), idMap);
    ...
    const newBranch = remapActionIds(templateBranch, idMap);
    ...
    payload.actions = nextActions.map((action, index) => index === host.actionIndex
    ...
    const preview = {
        ...
        emailWiring: emailActionIds.map((id, index) => ({
            step: index + 1,
            oldEmailId: actionsById.get(id)?.fields?.content_id,
            newEmailId: input.emailIds[index],
        })),
    };
    if (!apply) {
        return makeEnvelope(operation, { ...audit, attempted: false, verified: false }, { dryRun: true, preview, payload });
    }
    try {
        await updateWorkflow(input.workflowId, payload);
        const verified = await getWorkflow(input.workflowId);
        const verifiedActions = Array.isArray(verified.actions)
            ? verified.actions.filter(isRecord)
            : [];
        const landed = findTemplateBranch(verifiedActions, input.matchValue);
        if (!landed) {
            return makeErrorEnvelope({
                operation,
                error: new Error("HubSpot accepted the update but the new branch could not be read back."),
        ...
        const landedBranch = landed.action.listBranches[landed.branchIndex];
        const landedRoot = isRecord(landedBranch) &&
        ...
        const landedEmailIds = landedRoot
            ? collectChain(verifiedById, landedRoot)
                ...
                .map((id) => verifiedById.get(id)?.fields?.content_id)
            : [];
        const emailsMatch = landedEmailIds.length === input.emailIds.length &&
            landedEmailIds.every((id, index) => id === input.emailIds[index]);
        return makeEnvelope(operation, { ...audit, attempted: true, verified: emailsMatch }, {
            preview,
